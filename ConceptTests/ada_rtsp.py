...
class AdaptiveYOLOProcessor:
    ...
    def process_frame(self, frame, frame_count):
        ...
        # Set verbose=False for cleaner logs
    ...
    def process_rtsp_stream(self, input_rtsp, output_rtsp):
        ...
        print(
            f"Resolution: {frame_width}x{frame_height} @ {source_fps:.2f} FPS")
        ...
        if source_fps > 0:
            if self.output_fps > source_fps:
                ...
                print(
                    "Please set OUTPUT_FPS to a value less than or equal to the source FPS.")
                cap.release()
        ...
        while True:
            ...
            if not ret:
                print(
                    "Failed to grab frame from input stream. Attempting to reconnect...")
                cap.release()
                time.sleep(2)
                cap = cv2.VideoCapture(input_rtsp)
                continue
            ...
            wait_time = max(0, self.target_frame_time -
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run Adaptive YOLO RTSP Processor with Elasticsearch Logging")
    parser.add_argument("--input", required=True,
                        help="Path to the input RTSP stream.")
    parser.add_argument("--output", required=True,
                        help="Path for the output RTSP stream")
    parser.add_argument("--fps", type=float, required=True,
                        help="Target output FPS.")
    parser.add_argument("--upgrade_interval", type=int, default=20,
    ...
    model_paths = {
        "yolov5nu": "yolov5n.pt",   # Nano
        "yolov5su": "yolov5s.pt",   # Small
        "yolov5mu": "yolov5m.pt",   # Medium
        "yolov5lu": "yolov5l.pt",   # Large
        "yolov5xu": "yolov5x.pt",   # XLarge
    }
